...
export class QueryGraph {
  ...
  private createTableNode(node_info: TableNodeInfo): Node {
    ...
    const data: TableNodeData = {
      ...
      name:
        node_info.relationName +
        (alias && node_info.relationName !== alias ? ` AS ${alias}` : ""),
  ...
  private createScanNode(node_info: NodeInfo): Node {
    ...
    const filter = node_info.filter?.replace(/\$(\d)/g, (_, g) => {
    ...
    const attributes =
      node_info.output?.map((col) => {
        ...
        const attribute: Attribute = {
          ...
          type: type ?? "",
          keyType:
            isPk && isFk ? "PK, FK" : isPk ? "PK" : isFk ? "FK" : undefined,
        };
  ...
  public getGraph(node_info: NodeInfo[], table_nodes: TableNodeInfo[]): Graph {
    const nodes: Node[] = [];
    const edges: Edge[] = [];
